    ...
    public void onViewCreated(@Nullable View view, @Nullable Bundle savedInstanceState) {
        if (view != null) {
            ...
            if (attachmentBitmap != null) {
                ...
                GlideUrl glideUrl = new GlideUrl(imageData + "?token=" + App.getSharedPreferencesHandler().getEsriToken(),
                        new LazyHeaders.Builder()
                                .addHeader("User-Agent", "drppl")
                                .build());
                Glide.with(this).load(glideUrl).into(attachmentPreviewIv);
                // Added because attachments not loading due to Esri Token
                // Glide.with(this).load(imageData + "?token=" + App.getSharedPreferencesHandler().getEsriToken()).into(attachmentPreviewIv);
//                Picasso.get().load(imageData).into(attachmentPreviewIv);
//                if (imageData != null && !imageData.equals("")) {
//                    Picasso.get()
//                            .load(imageData) // web image url
//                            .rotate(90)
//                            .into(attachmentPreviewIv);
//                }
            }
